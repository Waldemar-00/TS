...
{
  server: string,
  game: Game, //string | number
  hours: Hours
  //| number
  //| string
  //| { total: number, inMenu: number }
...
function calculateAmountOfFigures ( figure: DataFigures[]): AmountOfFigures
{
  const amountFigures: AmountOfFigures = {
    squares: 0,
	  circles: 0,
	  triangles: 0,
	  others: 0,
  }
  figure.forEach( ( obj: DataFigures ) =>
  {
    switch (obj.name) {
      case 'rect':
